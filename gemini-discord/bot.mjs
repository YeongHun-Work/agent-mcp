 ...
 * @returns {{ name: string, url: string, description: string }[]}
 */
function loadSkills() {
  try {
...
/**
 * Gemini CLI를 spawn으로 실행하고 stdout을 반환한다.
...
function callGemini(prompt) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      proc.kill('SIGTERM');
      reject(new Error(`Gemini CLI 타임아웃 (${GEMINI_TIMEOUT_MS / 1000}초 초과)`));
    }, GEMINI_TIMEOUT_MS);
    ...
    proc.on('close', (code) => {
      ...
      if (code === 0) {
        ...
        reject(new Error(`Gemini CLI 종료 코드 ${code}: ${stderr.trim()}`));
      }
    });

    proc.on('error', (err) => {
      clearTimeout(timer);
      reject(new Error(`Gemini CLI 실행 실패: ${err.message}`));
    });
  });
}
...
/**
 * 채널 세션 컨텍스트와 topic을 이용해 Gemini 프롬프트를 구성하고 호출한다.
 * @param {string} channelId
 * @param {string} question
 * @returns {Promise<string>} - Gemini 응답
 */
async function askGemini(channelId, question) {
  const session      = getSession(channelId);
...
const commands = [
  new SlashCommandBuilder()
    .setName('ask')
    .setDescription('Gemini에게 질문합니다 (채널 세션 컨텍스트 유지)')
    .addStringOption((opt) =>
      opt
        .setName('question')
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (commandName === 'ask') {
    ...
    // Gemini 응답이 느릴 수 있으므로 defer 처리
    await interaction.deferReply();

    try {
      const answer = await askGemini(channelId, question);
  ...
  if (commandName === 'history') {
    ...
    // deferReply 없이 바로 응답 (DB 조회만이므로 빠름)
    await interaction.deferReply();
